...
function loginPage(prefix, next, message = "") {
  ...
  const safeNext = encodeURIComponent(next || `${prefix}/`);
...
(function () {
  ...
  function withPrefix(value) {
    if (!prefix || typeof value !== "string" || value.charAt(0) !== "/" || value.indexOf(prefix + "/") === 0 || value === prefix) return value;
    if (/^\\/(api|assets|plugins)(\\/|$)/.test(value) || value === "/favicon.svg" || value === "/manifest.webmanifest") return prefix + value;
    return value;
  }
  ...
  if (originalFetch) {
    globalThis.fetch = function (input, init) {
      ...
      else if (input && typeof input.url === "string") input = new Request(withPrefix(new URL(input.url).pathname + new URL(input.url).search), input);
...
export function apply(ctx) {
  ...
  ctx.webServer.tapIndex((html) => injectIndex(html, prefix));

  ctx.webServer.register({ kind: "prefix", path: "/fnos-access", handler: async (req, res) => {
    const url = new URL(req.url || "/", "http://fnos.local");
    ...
    const next = decodeURIComponent(url.searchParams.get("next") || `${prefix}/`);
    if (req.method === "GET") {
